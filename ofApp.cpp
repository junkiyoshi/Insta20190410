	...
	for (int x = len * -0.5; x < len * 0.5; x += span) {
		...
		for (int y = len * -0.5; y < len * 0.5; y += span) {
			...
			for (int i = 0; i < 4; i++) {
				...
				this->base_rects.push_back(rect);
			}
...
void ofApp::update() {
	...
	this->rects.clear();
	for (auto& base_rect : this->base_rects) {

		vector<glm::vec3> rect;
		for (auto& point : base_rect) {

			auto rotation_y = glm::rotate(glm::mat4(), ofMap(ofNoise(point.y * 0.001 + ofGetFrameNum() * 0.01), 0, 1, -PI, PI), glm::vec3(0, 1, 0));
			glm::vec3 location = glm::vec4(point, 0) * rotation_y;

			rect.push_back(location);
		}
